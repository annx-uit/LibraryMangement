...
@section('title', 'Quản lý thể loại - Hệ thống thư viện')
...
@section('content')
  <div class="container">
    <div class="header">
      <h1>📚 Hệ thống quản lý thể loại sách</h1>
      <p>Dành cho nhân viên thư viện</p>
    </div>

    <!-- Toast notifications -->
    <div id="toast" class="toast" style="display: none; position: fixed; top: 20px; right: 20px; background: #48bb78; color: white; padding: 15px 20px; border-radius: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.2); z-index: 1001;">
      <span id="toast-message"></span>
    </div>
    <div class="stats-grid">
      <div class="stat-card">
        <div class="stat-number" id="totalGenres">0</div>
        <div class="stat-label">Tổng số thể loại</div>
      </div>
    </div>
    <div class="controls">
      ...
      <div class="button-group">
        ...
        <button class="add-btn add-genre-btn" onclick="openModal()">➕ Thêm thể loại mới</button>
      </div>
    </div>
    <div class="table-container">
      ...
      <div class="empty-state" id="emptyState" style="display: none; text-align: center; padding: 60px 20px; color: #718096;">
        ...
        <p>Hãy thêm thể loại đầu tiên!</p>
      </div>
    </div>
  </div>
  <div class="modal" id="genreModal">
    <div class="modal-content">
      <h2 id="modalTitle">Thêm thể loại mới</h2>
      <form id="genreForm">
...
<script>
  ...
  function renderGenres() {
    ...
    if (filteredGenres.length === 0) {
      ...
      tbody.innerHTML = '';
      filteredGenres.forEach((g, idx) => {
        const tr = document.createElement('tr');
        tr.innerHTML = `
          <td>${g.name}</td>
          <td class="actions">
            <button class="btn edit-btn" onclick="openModal(${g.id})">✏️ Sửa</button>
            <button class="btn delete-btn" onclick="deleteGenre(${g.id})">🗑️ Xóa</button>
          </td>
        `;
  ...
  function searchGenres() {
    ...
    filteredGenres = term
      ? genres.filter(g => g.name.toLowerCase().includes(term))
      : [...genres];
    renderGenres();
  }
  ...
  async function saveGenre(e) {
    ...
    try {
      ...
      showToast('Lỗi khi lưu thể loại: ' + err.message, 'error');
    }
  }
  ...
  async function deleteGenre(id) {
    ...
    if (!confirm(`Bạn có chắc chắn xóa thể loại "${g.name}"?`)) return;
    
    try {
      const response = await goApi(`/theloai/${id}`, 'DELETE');
      if (response.success) {
