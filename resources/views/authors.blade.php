...
@section('title', 'Quản lý tác giả - Hệ thống thư viện')
...
<style>
    * {
    ...
    .nav-publishers {
      ...
      box-shadow: 0 4px 15px rgba(159, 122, 234, 0.2) !important;
    }
    
    .nav-publishers:hover {
      background: linear-gradient(135deg, #805ad5, #6b46c1) !important;
      box-shadow: 0 8px 25px rgba(128, 90, 213, 0.3) !important;
    }
...
@section('content')
  <div class="container">
    <div class="header">
      <h1>👤 Hệ thống quản lý tác giả</h1>
    ...
    <div class="stats-grid">
      <div class="stat-card">
        ...
        <div class="stat-label">Tổng số tác giả</div>
      </div>
    </div>
    
    <div class="controls">
      <div class="search-box">
        <input type="text" id="searchInput" placeholder="Tìm kiếm tác giả..." />
      ...
      <div class="button-group">
        ...
        <button class="add-btn add-author-btn" onclick="openModal()">➕ Thêm tác giả mới</button>
      </div>
    </div>
  ...
  <div class="modal" id="authorModal">
    <div class="modal-content">
      <h2 id="modalTitle">Thêm tác giả mới</h2>
      <form id="authorForm">
...
<script>
  ...
  function renderAuthors() {
    ...
    if (filteredAuthors.length === 0) {
      ...
      tbody.innerHTML = '';
      filteredAuthors.forEach((author, idx) => {
        const tr = document.createElement('tr');
        tr.innerHTML = `
          <td>${author.name}</td>
          <td class="actions">
            <button class="btn edit-btn" onclick="openModal(${author.id})">✏️ Sửa</button>
            <button class="btn delete-btn" onclick="deleteAuthor(${author.id})">🗑️ Xóa</button>
          </td>
        `;
  ...
  function searchAuthors() {
    ...
    filteredAuthors = term
      ? authors.filter(author => author.name.toLowerCase().includes(term))
      : [...authors];
    renderAuthors();
  }
  ...
  function openModal(id = null) {
    ...
    if (id) {
      ...
      if (author) {
        currentEditId = author.id;
        title.textContent = 'Sửa tác giả';
        input.value = author.name;
      }
    } else {
      title.textContent = 'Thêm tác giả mới';
  ...
  async function saveAuthor(e) {
    ...
    try {
      ...
      showToast('Lỗi khi lưu tác giả: ' + err.message, 'error');
    }
  }
  ...
  async function deleteAuthor(id) {
    ...
    if (!confirm(`Bạn có chắc chắn xóa tác giả "${author.name}"?`)) return;
    
    try {
      const response = await goApi(`/api/tacgia/${id}`, 'DELETE');
      if (response.success) {
        authors = authors.filter(a => a.id !== id);
        searchAuthors();
        showToast('Xóa tác giả thành công!');
      } else {
        showToast(response.message || 'Không thể xóa tác giả', 'error');
      }
    } catch (err) {
      console.error('Error:', err);
